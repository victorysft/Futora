...
export default function Dashboard() {
  ...
  if (loading) {
    return <div className="f-loading">Loading...</div>;
  }
  ...
  return (
    <>
      ...
      <div className="f-center">
        <h1 className="f-title">FUTORA</h1>
        <p className="f-subtitle">Your future is built by what you do today.</p>

        <div className="f-divider" />

        <span className="f-label">YOUR MISSION</span>
        <p className="f-identity">{identity || "Define your mission..."}</p>

        <div className="f-divider" />

        <span className="f-streak-number">{streak}</span>
        <span className="f-streak-label">day streak</span>

        {lastCheckIn && (
          <span className="f-last-checkin">Last check-in: {lastCheckIn}</span>
        )}
        ...
        <div className="f-footer">
          <button className="f-link" onClick={handleStartOver}>Start over</button>
          <span className="f-dot">·</span>
          <button className="f-link" onClick={handleSignOut}>Sign out</button>
        </div>
      </div>
    </>
  );
